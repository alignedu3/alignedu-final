...
const plans = [
  { name: 'Free', price: '$0', note: '2 analyses / month', offer: 'AI lesson intelligence · Core teaching insights', cta: 'Get started free' },
  { name: 'Teacher', price: '$14.99', cadence: '/month', annual: '$149/year', effective: '$12.42/mo', savings: 'Save $30.88/year', note: '10 analyses / month', offer: 'Everything in Free · History & insights', cta: 'Choose Teacher' },
  { name: 'Teacher Pro', price: '$24.99', cadence: '/month', annual: '$249/year', effective: '$20.75/mo', savings: 'Save $50.88/year', note: '30 analyses / month', offer: 'Everything in Teacher · Deeper insights', cta: 'Choose Teacher Pro', featured: true },
  { name: 'School / District', price: 'Custom', note: 'Organization access', offer: 'Team access · District monitoring', cta: 'Request School / District' },
];
...
export default function HomepagePlansTeaser() {
  ...
  const checkout = async (plan: 'teacher' | 'teacher_pro') => {
    setLoadingPlan(plan);
    try {
      const response = await fetch('/api/billing/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan, interval: 'monthly' }) });
        ...
        <div className="home-plans-grid">
          {plans.map((plan) => (
            <article key={plan.name} className={`home-plan-card ${plan.featured ? 'featured' : ''}`}>
              ...
              <div className="home-plan-offer">{plan.offer.split(' · ').map((item) => <span key={item}>✓ {item}</span>)}</div>
              {plan.annual ? <div className="home-plan-annual"><strong>{plan.annual}</strong><span>{plan.effective} effective</span>{plan.savings ? <em>{plan.savings}</em> : null}</div> : <div className="home-plan-annual placeholder"><span>{plan.name === 'Free' ? 'No credit card required' : 'Custom organization access'}</span></div>}
              <span className="home-plan-note">{plan.note}</span>
              {plan.name === 'Teacher' ? <button type="button" className="home-plan-cta primary" disabled={loadingPlan !== null} onClick={() => checkout('teacher')}>{loadingPlan === 'teacher' ? 'Opening checkout…' : plan.cta}</button> : plan.name === 'Teacher Pro' ? <button type="button" className="home-plan-cta primary" disabled={loadingPlan !== null} onClick={() => checkout('teacher_pro')}>{loadingPlan === 'teacher_pro' ? 'Opening checkout…' : plan.cta}</button> : <a href={plan.name === 'Free' ? '/signup' : 'mailto:support@alignedu.net?subject=AlignEDU%20School%20%2F%20District%20Demo%20Request&body=Hello%20AlignEDU%20Team%2C%0A%0AI%27m%20ready%20to%20take%20my%20school%20or%20district%20to%20the%20next%20level%20with%20AlignEDU%20and%20would%20like%20to%20schedule%20a%20demo.%0A%0APlease%20send%20me%20available%20times%20and%20next%20steps.%0A%0AThank%20you%21'} className="home-plan-cta primary">{plan.cta}</a>}
            </article>
          ))}
        </div>
