        ...
        <div className="home-plans-grid">
          {plans.map((plan) => (
            <article key={plan.name} className={`home-plan-card ${plan.featured ? 'featured' : ''}`}>
              ...
              <Link href={plan.name === 'Free' ? '/signup' : plan.name === 'School / District' ? 'mailto:support@alignedu.net?subject=AlignEDU School / District Access' : '/pricing'} className="home-plan-cta primary">{plan.cta}</Link>
            </article>
          ))}
        </div>
