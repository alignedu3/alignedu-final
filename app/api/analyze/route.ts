...
import {
  generateStructuredAnalysisReport,
...
async function updateAnalysisJob(
  ...
  if (error && /column .* does not exist/i.test(error.message || "")) {
    ...
    const { error: retryError } = await serviceSupabase
      .from("analysis_jobs")
      .update(legacyCompatiblePatch)
      .eq("id", jobId);

    if (!retryError) {
      console.warn("ANALYSIS JOB UPDATE USED LEGACY PATCH:", { jobId });
      return;
    }

    console.error("ANALYSIS JOB LEGACY PATCH RETRY ERROR:", {
      jobId,
      error: retryError,
      patch: legacyCompatiblePatch,
    });
    return;
  }
...
\nTranscript:\n${transcript}\n`;
  ...
  let result = "";
  let generationDiagnostics: StructuredAnalysisGenerationDiagnostics | null = null;
  let fallbackReason: string | null = null;
  try {
    const structured = await generateStructuredAnalysisReport({
      openai,
      systemPrompt,
      userPrompt,
      higherEdAlignmentTitle: isHigherEdCustomText
        ? "HIGHER ED TEXTBOOK ALIGNMENT"
        : isHigherEdBiology
          ? "HIGHER ED BIOLOGY TEXTBOOK ALIGNMENT"
          : undefined,
    });
    result = structured.reportText;
    generationDiagnostics = structured.diagnostics;
  } catch (structuredError) {
    console.warn("STRUCTURED ANALYSIS FALLBACK:", structuredError);
    fallbackReason = getErrorMessage(structuredError, "Structured analysis failed.");
    const completion = await callOpenAI(openai, [
      { role: "system", content: systemPrompt },
      { role: "user", content: userPrompt },
    ]);

    result = completion.choices[0]?.message?.content || "No result returned";
  }
