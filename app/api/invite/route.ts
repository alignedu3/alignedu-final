...
export async function POST(req: Request) {
  const { name, email, role, linkedAdminId } = await req.json();

  if (!name || !email || !role) {
    return new Response(JSON.stringify({ error: 'name, email, and role are required.' }), { status: 400 });
  }
  if (role === 'teacher' && !linkedAdminId) {
    return new Response(JSON.stringify({ error: 'linkedAdminId is required when inviting a teacher.' }), { status: 400 });
  }
