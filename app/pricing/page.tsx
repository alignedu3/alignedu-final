...
const plans = [
  ...
  { key: 'school', name: 'School / District', price: 'Custom', annual: '', annualMonthly: '', savings: '', limit: 'Team access + pooled/custom usage', copy: 'For instructional leaders, campuses and districts.', features: ['Administrator dashboards', 'Invite & manage authorized users', 'Organization-level visibility'], featured: false },
] as const;
    ...
    <section className="grid">
      {plans.map((plan) => <article key={plan.key} className={`card ${plan.featured ? 'featured' : ''}`}>
        ...
        <div className="card-action">
          {plan.key === 'free' && <Link className="cta secondary" href="/login">Get started free</Link>}
          {plan.key === 'teacher' && <button type="button" className="cta secondary" disabled={loadingPlan !== null} onClick={() => checkout('teacher')}>{loadingPlan === 'teacher' ? 'Opening checkout…' : annual ? 'Get Teacher annually' : 'Get Teacher'}</button>}
