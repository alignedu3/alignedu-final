    ...
    <section className="pricing-hero">
      ...
      <div className="toggle-shell">
        <div className="toggle">
          ...
          <button type="button" className={annual ? 'active' : ''} onClick={() => setAnnual(true)}>Annual <span>Save</span></button>
        </div>
      </div>
    </section>
    ...
    <section className="grid">
      {plans.map((plan) => <article key={plan.key} className={`card ${plan.featured ? 'featured' : ''}`}>
        ...
        <div className="card-action">
          {plan.key === 'free' && <Link className="cta secondary" href="/login">Get started</Link>}
          {plan.key === 'teacher' && <button className="cta secondary" disabled={loadingPlan !== null} onClick={() => checkout('teacher')}>{loadingPlan === 'teacher' ? 'Opening checkout…' : 'Choose Teacher'}</button>}
          {plan.key === 'teacher_pro' && <button className="cta primary" disabled={loadingPlan !== null} onClick={() => checkout('teacher_pro')}>{loadingPlan === 'teacher_pro' ? 'Opening checkout…' : 'Choose Teacher Pro'}</button>}
          {plan.key === 'school' && <a className="cta secondary" href="mailto:support@alignedu.net?subject=AlignEDU School or District Plan&body=I would like to learn more about AlignEDU School / District pricing and access.">Contact AlignEDU</a>}
        </div>
      </article>)}
    </section>
